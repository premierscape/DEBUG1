...
public enum CreditHandler {
	...
	DONATOR(205065, 0, new Handle() {
		...
		public void handle(Player player) {
			if (!player.inWilderness()) {
			player.start(new OptionDialogue("GrandPremier Bucks Store 1", p -> {
				player.getShopping().open(94);
			} , "GrandPremier Bucks Store 2", p -> {
				player.getShopping().open(90);
			} , "GrandPremier Bucks Store 3", p -> {
	...
	public static boolean allowed(Player player, CreditPurchase credit, int amount) {
		...
		if (player.getCredits() < amount) {
			DialogueManager.sendStatement(player, "@red@You do not have enough GrandPremier Bucks to do this!");
			player.send(new SendMessage("Please visit @red@http://www.GrandPremier.net/store/ </col>to purchase more GrandPremier Bucks!"));
			return false;
		}
		if (player.inWilderness()) {
	...
	public static void spent(Player player, int amount) {
		player.send(new SendMessage("@blu@You have spent " + amount + " GrandPremier Bucks; Remaining: " + player.getCredits() + "."));
		player.getClient().queueOutgoingPacket(new SendString("</col>GrandPremier Bucks: @gre@" + Utility.format(player.getCredits()), 52504));	
