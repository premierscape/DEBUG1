...
public class PlayerCombatInterface implements CombatInterface {
	...
	public void onHit(Entity entity, Hit hit) {
		...
		if (player.getAttributes().get(MiniRaidGame.MINIRAID_GAME_KEY) != null) {
			player.getAttributes().set(MiniRaidGame.MONSTER_DAMAGE_KEY, player.getAttributes().get(MiniRaidGame.MONSTER_DAMAGE_KEY) != null ? player.getAttributes().getInt(MiniRaidGame.MONSTER_DAMAGE_KEY) + hit.getDamage() : hit.getDamage());
		}
		
		if (player.getAttributes().get(MiniRaid2Game.MINIRAID2_GAME_KEY) != null) {
			player.getAttributes().set(MiniRaid2Game.MONSTER_DAMAGE_KEY, player.getAttributes().get(MiniRaid2Game.MONSTER_DAMAGE_KEY) != null ? player.getAttributes().getInt(MiniRaid2Game.MONSTER_DAMAGE_KEY) + hit.getDamage() : hit.getDamage());
		}
