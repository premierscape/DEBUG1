...
public class TutorialNpcMoney extends Dialogue {
	...
	public void execute() {
		...
		case 5:
			...
			break;
		case 6:
			nChat(new String[] { "Like every skill on GrandPremier,", "You will receive skill points to be spent", "in the skill points shop on useful supplies!" });
			break;
		case 7:
			tele(1643, 3674);
			nChat(new String[] { "There are MANY more ways to get rich on GrandPremier.", "You can farm clue scrolls from monsters.", "Loads of @blu@NEW items</col> have been added to clues." });
			break;
		case 8:
			nChat(new String[] { "Monsters around GrandPremier also drop Crystal Keys commonly", "and with the crystal chest now packed with 100s", "of new items, it's a great way to make BANK!" });
			break;
		case 9:
			nChat(new String[] { "Bloodmoney is also a second currency on GrandPremier,", "with many of its own shops", "to purchase items." });
			break;
		case 10:
			nChat(new String[] { "You receive blood money for doing many things", "around GrandPremier such as skills, killing monsters", "killing other players & random drops." });
			break;
		case 11:
