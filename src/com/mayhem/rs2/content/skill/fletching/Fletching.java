...
public enum Fletching {
	...
	public boolean start(Player player, Fletchable fletchable, int index, int amount) {
		...
		TaskQueue.queue(new Task(player, 2, true, StackType.NEVER_STACK, BreakType.ON_MOVE, TaskIdentifier.SKILL_CREATING) {
			...
			public void execute() {
				...
				if (fletchable == (Stringable.MAGIC_LONGBOWBOWS)) {
				player.getDailyChallenges().deincrementChallenge(DailyChallenges.MAGIC_LONGS);
				}
				if (fletchable == (Stringable.YEW_LONGBOWBOWS)) {
				player.getDailyChallenges().deincrementChallenge(DailyChallenges.YEW_SHORT);
				}
	...
	public static void handleBloodMoney(Player player) {
		if (player.getInventory().hasSpaceFor(new Item(13307))) {
			player.getInventory().add(new Item(13307, Utility.randomNumber(10)));
		} else if (player.getBank().hasSpaceFor((new Item(13307)))) {
			player.getBank().add((new Item(13307, Utility.randomNumber(10))));
