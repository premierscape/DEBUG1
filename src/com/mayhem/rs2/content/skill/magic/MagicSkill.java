...
public class MagicSkill {
	...
	public boolean canTeleport(TeleportTypes type) {
		...
		if (player.getController().equals(ControllerManager.MINI_RAID_WAITING_ROOM_CONTROLLER)) {
			player.getClient().queueOutgoingPacket(new SendMessage("Please Exit the boat via the portal."));
			return false;
		} else if (player.getController().equals(ControllerManager.MINIRAID_CONTROLLER)) {
			player.getClient().queueOutgoingPacket(new SendMessage("You can't teleport whilst in the mini raid."));
			player.getClient().queueOutgoingPacket(new SendMessage("If you wish to leave use the portal."));
			return false;
		}
		else if (player.getController().equals(ControllerManager.MINIRAID2_CONTROLLER)) {
			player.getClient().queueOutgoingPacket(new SendMessage("You can't teleport whilst in the mini raid 2."));
			player.getClient().queueOutgoingPacket(new SendMessage("If you wish to leave use the portal."));
			return false;
		}
	...
	public boolean needsRune(int runeId) {
		...
		case 556:
			...
			return true;
		case 554:
            if ((wep == 3053) ||(wep == 1387) || (wep == 1393) || (wep == 17293)) {
                return false;
            } 
			return true;
		case 555:
			...
			return true;
		case 557:
            if ((wep == 1385) || (wep == 20736) || (wep == 20739) || (wep == 1399) || (wep == 17293) || (wep == 3053)) {
                return false;
            }
			return true;
		}
