			...
			if (active(Prayer.PROTECT_FROM_MELEE)) {
				...
				if (target != null) {
					if (target.isNpc()) {
						...
						if (id == 1498) {
							return (int) (hit.getDamage() * 0.95D);
						}
						if (id == 1540) {
							return hit.getDamage() / 2;
						}
						...
						if (id == 2043) {
							return (int) (hit.getDamage() * 0.25D);
						}
						
						if (mob.getId() == 3129) {
							return (int) (hit.getDamage() *0.30D);
						}
						...
						if (mob.getId() == 5862) {
							return (int) (hit.getDamage() * 0.60D);
						}
		...
		case MAGIC:
			if (active(Prayer.PROTECT_FROM_MAGIC)) {
				...
				if ((target != null) && (target.isNpc())) {
					...
					if (mob.getId() == 7538) {
						return (int) (hit.getDamage() * 0.50D);
					}
					if (mob.getId() == 1498) {
						return (int) (hit.getDamage() * 0.70D);
					}
					...
					if (mob.getId() == 3129) {
						return (int) (hit.getDamage() *0.20D);
					}
		...
		case RANGED:
			if (active(Prayer.PROTECT_FROM_RANGE)) {
				...
				if (target != null && target.isNpc()) {
					...
					if (id == 3474) {
						return (int) (hit.getDamage() * 0.10D);
					}
