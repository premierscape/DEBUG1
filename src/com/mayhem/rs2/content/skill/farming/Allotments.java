...
public class Allotments {
	...
	public static final int FALADOR_AND_PHASMATYS_CONFIG = 529;
	public static final int ARDOUGNE_CONFIG = 529;
	public static final int CATHERBY_CONFIG = 529;
	...
	public void updateAllotmentsStates() {
		...
		configValue = configValues[0] + (configValues[ 1+ 1] << 8);
		player.send(new SendConfig(ARDOUGNE_CONFIG, configValue));
		
		configValue = configValues[2] | (configValues[3 + 1] << 8) ;
		player.send(new SendConfig(FALADOR_AND_PHASMATYS_CONFIG, configValue));
		
		configValue = (configValues[4] << 8) | configValues[5 + 1];
		player.send(new SendConfig(CATHERBY_CONFIG, configValue));
	...
	public static void handleERing1(Player player) {

		int random = Utility.randomNumber(200);
		if (random == 1) {
			if (player.getEquipment().isWearingItem(13125, 1)) {
				player.getEquipment().remove(new Item(13125, 1));
				player.getEquipment().equip(new Item(13126), 1);
				player.setAppearanceUpdateRequired(true);
				player.getUpdateFlags().setUpdateRequired(true);
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@Your Explorers ring has been upgraded!")); }
			}
		}

	public static void handleERing2(Player player) {

		int random = Utility.randomNumber(300);
		if (random == 1) {
			if (player.getEquipment().isWearingItem(13126, 1)) {
				player.getEquipment().remove(new Item(13126, 1));
				player.getEquipment().equip(new Item(13127), 1);
				player.setAppearanceUpdateRequired(true);
				player.getUpdateFlags().setUpdateRequired(true);
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@Your Explorers ring has been upgraded!")); }
		}
	}
	public static void handleERing3(Player player) {

		int random = Utility.randomNumber(1000);
		if (random == 1) {
			if (player.getEquipment().isWearingItem(13127, 1)) {
				player.getEquipment().remove(new Item(13127, 1));
				player.getEquipment().equip(new Item(13128), 1);
				player.setAppearanceUpdateRequired(true);
				player.getUpdateFlags().setUpdateRequired(true);
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@Your Explorers ring has been upgraded!")); }
		}
