...
public class InputFieldPacket extends IncomingPacket {
	...
	public void handle(Player player, StreamBuffer.InBuffer in, int opcode, int length) {
		...
		case 38090: //Wogw donation amount
			player.wogwDonationAmount = Long.parseLong(text);
			if (player.wogwOption.isEmpty()) {
				player.send(new SendMessage("You must choose something to donate towards click one of the options above."));
				return;
			}
			player.send(new SendInterface(38_020));
			player.send(new SendString("Are you sure you want to contribute\\n" + Utility.getValueWithoutRepresentation(player.wogwDonationAmount) + " gp?", 38022));
			break;
