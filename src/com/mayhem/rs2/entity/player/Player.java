...
public class Player extends Entity {
	...
	public void addDefaultChannel() {
		if (clan == null) {
			Clan localClan = Server.clanManager.getClan("brandon");
			if (localClan != null)
	...
	public boolean login(boolean starter) throws Exception {
		...
		send(new SendString("</col>GrandPremier Bucks: @gre@" + Utility.format(this.getCredits()), 52504));
		...
		if (getRights() == 4) {
			World.sendGlobalMessage("<img=3>@dre@Founder/Developer " + username + " has logged in!");
		} 		
		else if (getRights() == 3) {
			World.sendGlobalMessage("<img=3>@dre@Owner " + username + " has logged in!");
		}
		else if (getRights() == 2) {
			World.sendGlobalMessage("<img=1><col=D17417>Administrator " + username + " has logged in!</col>");
		}
		else if (getRights() == 1) {
			World.sendGlobalMessage("<img=0>@blu@Moderator " + username + " has logged in!");
		}
		else if (getRights() == 13) {
			World.sendGlobalMessage("<img=12>@blu@Support " + username + " has logged in!");
		}
		...
		send(new SendMessage("<img=8>@blu@Welcome to GrandPremier!"));
		send(new SendMessage("<img=8>@blu@Make sure you register on our active community forums: www.premierscape.net"));
		send(new SendMessage("<img=8>@blu@Don't forget to ::Vote every 12 hours for great rewards and to help the server grow!"));
		send(new SendMessage("<img=8>@dre@::Discord to join our Group chat! || Type ::Donate to visit our online store. "));
		
		
		if (World.getActivePlayers() > Constants.MOST_ONLINE) {
			Constants.MOST_ONLINE = World.getActivePlayers();
			FileHandler.saveMaxPlayers();
			World.sendGlobalMessage("[<col=910D0D>GrandPremier</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
		}
	...
	public void logout(boolean force) {
		if (isActive()) {
			...
			String key = "npzvkf1degh4hg218115opqfrz8l0i3aidnqmmc6lqx4ezsemio3k3962ep9dpkozgd5p6tj4i";
